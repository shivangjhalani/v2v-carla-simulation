...
class SimulationConfig:
    num_vehicles: int
    tick_rate: float
    control_mode: str = "autopilot"  # Default to autopilot mode
    port: int = 2000
    timeout: float = 10.0
    ...
    def validate(self):
        ...
        if self.control_mode not in ["manual", "autopilot"]:
            raise ValueError("control_mode must be either 'manual' or 'autopilot'")
...
class SimulationManager:
    ...
    def run(self):
        ...
        try:
            ...
            self.vehicle_controller = VehicleController(self.world, self.vehicle_manager, self)
            
            # Set initial autopilot state based on control mode
            for vehicle in vehicles:
                vehicle.set_autopilot(self.sim_config.control_mode == "autopilot")
            ...
            while self.running:
                ...
                if not self.vehicle_controller.handle_input(self.sim_config.control_mode):
                    self.running = False
                    break
                
                # Update vehicle states and dashboards
    ...
    def _print_control_instructions(self):
        ...
        print("\nControl Mode:", self.sim_config.control_mode.upper())
        print("\nAvailable Controls:")
        
        if self.sim_config.control_mode == "autopilot":
            print("Press 1-9 to follow respective vehicle")
            print("Press ESC to stop following")
        else:  # manual mode
            print("Press 1-9 to control respective vehicle")
            print("W/S - Throttle/Brake")
            print("A/D - Steering")
            print("SPACE - Handbrake")
            print("Q - Toggle Reverse")
            print("ESC - Release control")
